...
def process_image_with_grok(base64_image: str) -> dict:
    ...
    if response.status_code != 200:
        raise HTTPException(status_code=500, detail="Error processing the image with Grok.")
...
def process_chat_with_grok(messages: List[dict]) -> str:
    headers = {"Authorization": f"Bearer {XAI_API_KEY}"}
    payload = {"model": CHAT_MODEL_NAME, "messages": messages}
    response = requests.post("https://api.x.ai/v1/chat/completions", json=payload, headers=headers)
    if response.status_code != 200:
        raise HTTPException(status_code=500, detail="Error processing the chat with Grok.")
